...
form.addEventListener("submit", function (e) {
  e.preventDefault();
  //parsing the value to integer on submission 
  const value = parseInt(amount.value);
  const random = Math.floor(Math.random() * text.length);

  //empty value : not a number or is a -ve number or greater than 9
  if (isNaN(value) || value <= 0 || value > 9) {
    // then a  single para is displayed in random order
    result.innerHTML = `<p class="result">${text[random]}</p>`;
  } else {

    //slices the text array from 0 to the value creating a swallow copy of the og array 
    let tempText = text.slice(0, value);
    //to transform each element of the array into paragraph element
